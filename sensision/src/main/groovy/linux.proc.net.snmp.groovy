...
/**
 * Produce metrics about network statistics available via snmp
 */
...
try {
  ...
  File OUTFILE = getMetricsFile('linux.proc.net.snmp');
  ...
  while(true) {
    ...
    if (!tokens[0].equals(lastprefix)) {
      ...
      for (int i = 1; i < tokens.length; i++) {
        ...
        storeMetric(pw, now, "linux.proc.net.snmp.${protocol}.${varnames.get(i)}", labels, value);
      }
    }
  }
